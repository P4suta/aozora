...
#            is not provably play / vscode / book / infra lands here, so the
#            heavy gates run unless the change is confined to an isolated area.
#   play   — playground/** (the isolated Solid + Vite + WASM frontend).
#   vscode — editors/vscode/** (the isolated TypeScript VS Code extension).
#            Scoped to the one extension rather than editors/**: a sibling
#            editor would be a different project with different gates, and
#            must fall to `code` instead of riding this bucket's skips.
...
code=0 play=0 vscode=0 book=0 infra=0
while IFS= read -r f; do
    [[ -z "$f" ]] && continue
    case "$f" in
        playground/*) play=1 ;;
        editors/vscode/*) vscode=1 ;;
