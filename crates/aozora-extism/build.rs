...
//! via `rustc-env` so the Extism `version()` plugin export can read it via
...
//!      `{CARGO_PKG_VERSION}-dev+g{sha}[.dirty]`, so a hand-built plugin can never
...
//!   3. Otherwise — a packaged build with no `.git` (a release-workflow artifact
//!      build) → the clean `{CARGO_PKG_VERSION}`.
//!
//! This mirrors `aozora-wasm/build.rs` and `aozora-py/build.rs` — every binding
//! stamp shares one version-string format so every driver's `version()` reports
//! identically.
//!
...
//! plugin's incremental builds fast.
...
/// is this crate (`crates/aozora-extism`); the workspace root is two levels up.
/// A packaged build has no such directory.
