...
mod tests {
    ...
    /// UTF-8 leading bytes that may begin a trigger character — the bound over
    /// which `classify_match_and_trigram_array_cannot_drift` sweeps the reverse
    /// direction. Pinned complete against [`ALL_TRIGGER_TRIGRAMS`] by
    /// `trigger_leading_bytes_are_complete_for_known_triggers`.
    const TRIGGER_LEADING_BYTES: [u8; 3] = [0xE2, 0xE3, 0xEF];

    /// UTF-8 middle bytes (2nd byte of the trigram) covering every trigger.
