...
//!
//! Single authority (the [`crate::syntax::accent`] precedent): the pipeline
//! lint, the `aozora fmt --fix` autofix, and the LSP quick-fix all resolve
//! the canonical form here, on the trimmed body string, with no
//! `DirectiveKind` dependency.
...
/// [`crate::syntax::degraded`] — they fold measurement vocabularies, not spellings.)
...
mod tests {
    ...
    fn exact_maps_resolve() {
        ...
        // [`crate::syntax::degraded`] (Tier2, opt-in render only). Pin the boundary.
