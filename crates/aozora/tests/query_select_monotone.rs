//! Selector monotonicity: a more permissive query selects a superset of a more
//! restrictive one.
//!
//! Drives the same intuition the tree-sitter query language gives: a match on
//! `(_)` (any node) must include every match on a specific kind. A regression
//! that silently drops captures from `(_)` would make editor surfaces (LSP
//! highlight, "find all ruby") miss results — the proptest is the decisive
//! killer.
//!
//! Two angles:
//!
//! 1. `(_)` captures every node a specific-kind query captures (subset
//!    relation).
//! 2. Two independent runs of `captures` on the same query + tree yield the
//!    same result count (determinism).
#![cfg(feature = "query")]
...
fn assert_any_captures_superset_of_specific(source: &str) {
    ...
    // Every Construct capture's node must also appear in the any capture set. We
    // compare on text_range — `NodeOrToken` doesn't implement Hash/Eq cheaply
    // for set comparison.
...
proptest! {
    ...
    /// `(_)` is a superset of `(Construct)` over the workhorse Aozora fragment
    /// generator — input shape independence.
    #[test]
    fn aozora_fragment_any_superset_of_construct(s in aozora_fragment(120)) {
        assert_any_captures_superset_of_specific(&s);
    }

    /// Capture count is deterministic — running the same query twice against
    /// the same CST returns the same number of captures.
    #[test]
    fn aozora_fragment_captures_count_is_deterministic(s in aozora_fragment(120)) {
        assert_capture_count_is_deterministic(&s);
    }

    /// Pathological / unbalanced inputs — query execution stays total even when
    /// the lex pipeline emits diagnostics.
    #[test]
