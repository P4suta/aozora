...
///
/// The conservative trigger set (see [`AOZORA_TRIGGERS`]) plus the
...
/// `\r` (CRLF folding), `\u{FEFF}` (BOM), and the eight
/// [`AOZORA_TRIGGERS`] cover every transform / recognition path.
fn is_plain_phase0_clean(text: &str) -> bool {
    !text.contains('\r')
        && !text.contains('\u{FEFF}')
        && aozora_triggers_absent(text)
}

/// Aozora trigger characters — the union of every glyph any
/// classifier phase looks at first. A buffer free of these is
/// guaranteed to lex as a single `Plain` span.
const AOZORA_TRIGGERS: &[char] = &['［', '］', '《', '》', '｜', '※', '〔', '〕'];

/// True iff `text` contains none of [`AOZORA_TRIGGERS`].
fn aozora_triggers_absent(text: &str) -> bool {
    !text.contains(AOZORA_TRIGGERS)
}
