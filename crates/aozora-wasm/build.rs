...
//! via `rustc-env` so the wasm-bindgen `version()` export can read it via
...
//! binary's incremental builds fast.
...
/// is this crate (`crates/aozora-wasm`); the workspace root is two levels up.
