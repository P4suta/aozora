...
//! rope-backed impl, kept in the LSP layer so `ropey` never enters the core
//! `aozora` dependency tree. It lets [`crate::lsp::parse_cache::ParseCache`] hold
...
use aozora::{SanitizedSrc, SanitizedSrcSealed};
use ropey::RopeSlice;
...
impl<'a> RopeSrc<'a> {
    ...
    #[must_use]
    pub(super) fn new(slice: RopeSlice<'a>) -> Self {
        Self {
            slice,
            memo: Cell::new(None),
        }
...
// The `SanitizedSrc` seal is a `#[doc(hidden)]`, `unstable-internals`-gated
// supertrait: it keeps external crates from implementing the byte source while
// letting this in-workspace LSP (which enables the feature) supply the
// rope-backed one.
impl SanitizedSrcSealed for RopeSrc<'_> {}
