...
//! [`crate::lsp::line_index::LineIndex`]. Editing a single paragraph never
...
use crate::lsp::gaiji_spans::{GaijiSpan, extract_gaiji_spans_from_tree};
...
pub(super) const MAX_PARAGRAPH_BYTES: usize = 64 * 1024;

/// Mutable single-paragraph state. Owned by [`crate::lsp::state::DocBuffer`].
#[derive(Debug)]
pub(super) struct ParagraphBuffer {
    pub text: Rope,
...
impl ParagraphBuffer {
    ...
    #[must_use]
    pub(super) const fn new(text: Rope) -> Self {
        Self { text, tree: None }
    }
    ...
    pub(super) fn reparse(&mut self, parser: &mut Parser) {
    ...
    pub(super) fn apply_edit(&mut self, parser: &mut Parser, edit: InputEdit) {
    ...
    #[must_use]
    pub(super) fn tree_id(&self) -> Option<usize> {
...
///
/// Held inside `Arc` inside [`crate::lsp::state::DocSnapshot::paragraphs`].
...
#[derive(Debug)]
pub(super) struct ParagraphSnapshot {
    ...
    /// [`crate::lsp::semantic_tokens::semantic_tokens_full`] tracks while
    ...
    /// `crate::lsp::state::DocSnapshot::rebuild_now`).
...
impl ParagraphSnapshot {
    ...
    #[must_use]
    pub(super) fn shifted_to(prior: &Arc<Self>, new_start: usize) -> Arc<Self> {
