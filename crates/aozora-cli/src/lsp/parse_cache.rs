...
use aozora::unstable::LexOutput;
#[cfg(test)]
use aozora::unstable::Sentinel;
use aozora::unstable::sanitize::{is_rule_line_trimmed, sanitize};
use aozora::{DiagBaseRef, DiagSplice, Diagnostic, Document, PieceSeq, Tree};
use ropey::{Rope, RopeSlice};
use tracing::field::Empty as TracingEmpty;

use crate::lsp::rope_src::RopeSrc;
...
impl ParseCache {
    ...
    pub(super) fn with_tree<R>(&self, f: impl FnOnce(&Tree<'_>) -> R) -> Option<R> {
    ...
    /// The stored sanitized buffer of the most recent parse (a sanitize fixed
    /// point), or `None` before the first parse / for an empty or oversized
    /// document. The `incremental_rope_e2e` and shuttle (`shuttle_doc_state`)
    /// tests read it to assert the spliced sanitized rope stayed byte-equal to a
    /// fresh `sanitize` (#237 Tier 2, Mechanism B). Test-only so `dead_code`
    /// stays quiet in the shipped build.
    #[cfg(test)]
    pub(super) fn sanitized(&self) -> Option<&Rope> {
