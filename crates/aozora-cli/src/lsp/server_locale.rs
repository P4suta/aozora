...
//!
//! [`init_ui_lang`] fixes the value, once, from [`crate::lsp::backend`]'s
...
//! in isolation; only the [`crate::lsp::backend`] handlers reach for [`ui_lang`],
//! the single resolved value they thread down. The lookup itself goes through
//! the shared [`crate::i18n`] catalog — the same `.ftl` bundles the CLI uses,
//! so there is one place to translate a string, not two.

use std::env;
use std::sync::OnceLock;

use crate::i18n::{LanguageIdentifier, resolve as negotiate};
...
/// occasionally a tag we have no catalog for; `crate::i18n::resolve` parses,
...
#[must_use]
pub(super) fn ui_lang() -> &'static LanguageIdentifier {
