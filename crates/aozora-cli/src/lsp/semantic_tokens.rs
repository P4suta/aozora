...
use crate::lsp::paragraph::ParagraphSnapshot;
...
/// references keeps [`walk_paragraph_tree`] / [`token_at`] under
...
/// count.
#[must_use]
pub(super) fn semantic_tokens_full(paragraphs: &[Arc<ParagraphSnapshot>]) -> SemanticTokens {
...
fn walk_paragraph_tree(tree: &Tree, ctx: &ParagraphCtx<'_>, out: &mut Vec<RawToken>) {
    ...
    'walk: loop {
        ...
        if node.is_error() {
            ...
            match node.kind() {
                kind::GAIJI => {
                    out.push(token_at(node, ctx, TT_GAIJI));
                }
                kind::RUBY_BASE_EXPLICIT | kind::RUBY_BASE_IMPLICIT => {
                    if is_inside_ruby(node) {
                        out.push(token_at(node, ctx, TT_RUBY_BASE));
                    }
                }
                kind::RUBY_READING => {
                    if is_inside_ruby(node) {
                        out.push(token_at(node, ctx, TT_RUBY_READING));
                    }
                }
                _ => {
...
/// A semantic token in doc-absolute coordinates. Every kind we emit
/// (gaiji, ruby base / reading) has a newline-free grammar body, so a
/// token never crosses a line — which is exactly the LSP invariant that a
/// semantic token is single-line. The assert is the tripwire at the seam
/// with the grammar contract: it never fires for grammar-conformant input,
/// and a future newline-spanning token kind would trip it in tests.
fn token_at(node: tree_sitter::Node<'_>, ctx: &ParagraphCtx<'_>, token_type: u32) -> RawToken {
    let start = ctx.line_index.position(ctx.text, node.start_byte());
    let end = ctx.line_index.position(ctx.text, node.end_byte());
    debug_assert_eq!(
        start.line, end.line,
        "semantic tokens are single-line by grammar",
    );
    RawToken {
        line: start.line.saturating_add(ctx.line_offset),
...
mod tests {
    ...
    use crate::lsp::paragraph::{ParagraphBuffer, build_paragraph_snapshot, paragraph_byte_ranges};
