...
//! the editor-burst microbenchmarks and as the home of the `input_edit` helper that
//! [`crate::lsp::state::DocBuffer`] reuses.
//!
//! ## Production state lives in `crate::lsp::paragraph`, not here
//!
//! Each open document is split at `\n\n` boundaries into a
//! `Vec<ParagraphBuffer>`, where every [`crate::lsp::paragraph::ParagraphBuffer`]
...
//! slow for keystroke responsiveness. See [`crate::lsp::state`] for the
...
//! [`crate::lsp::paragraph::ParagraphBuffer::apply_edit`].
//!
...
//! [`crate::lsp::parse_cache::ParseCache`]). Tree-sitter is the
...
// The tree-sitter document oracle below is a differential test aid: the server
// parses per-paragraph (see `paragraph.rs`), so `TreeSitterDoc` — a whole-doc
// rope parser — exists only to cross-check that model in the in-module tests.
#[cfg(test)]
use ropey::Rope;
#[cfg(test)]
use std::fmt;
...
#[cfg(test)]
pub(crate) struct TreeSitterDoc {
    inner: Mutex<Inner>,
}
...
impl TreeSitterDoc {
    ...
    pub(crate) fn parse_full(&self, text: &str) {
    ...
    pub(crate) fn apply_edit(&self, new_text: &str, edit: InputEdit) {
        let mut inner = self.inner.lock().expect("parser mutex");
    ...
    pub(crate) fn parse_full_rope(&self, rope: &Rope) {
        let mut inner = self.inner.lock().expect("parser mutex");
        inner.tree = inner
            .parser
    ...
    /// # Panics
    /// On a poisoned inner `Mutex`.
    pub(crate) fn apply_edit_rope(&self, rope: &Rope, edit: InputEdit) {
        let mut inner = self.inner.lock().expect("parser mutex");
        let mut prior = inner.tree.take();
        if let Some(tree) = prior.as_mut() {
            tree.edit(&edit);
    ...
    /// # Panics
    /// On a poisoned inner `Mutex` (see [`Self::parse_full`]).
    pub(crate) fn with_tree<R>(&self, f: impl FnOnce(&Tree) -> R) -> Option<R> {
...
/// `crate::lsp::position`, private).
#[must_use]
pub(super) fn input_edit(start_byte: usize, old_end_byte: usize, new_end_byte: usize) -> InputEdit {
    InputEdit {
        start_byte,
        old_end_byte,
        new_end_byte,
