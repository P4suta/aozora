...
//! # Running this test
//!
//! `cargo test -p aozora-cli --features shuttle-tests` (or `just
//! test-internals`, which enables `--all-features`).
//!
//! Without the feature flag the module compiles out so default
...
/// `Document::new(text).parse().to_source().len()` keeps us exercising
...
//   own test suite + the concurrent-LSP integration tests).
