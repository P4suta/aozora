...
/// Find `program` on `PATH`, returning the first executable candidate — a
/// hand-rolled `which` so no dependency is added for one small probe.
fn which(program: &str) -> Option<PathBuf> {
    find_on_path(&env::var_os("PATH")?, program)
}

/// The pure PATH search behind [`which`], taking the raw `PATH` value so it is
/// unit-testable without mutating the process environment. Uses
/// [`env::split_paths`] for the platform's separator and, on Windows, the
/// `PATHEXT` extensions.
fn find_on_path(paths: &OsStr, program: &str) -> Option<PathBuf> {
