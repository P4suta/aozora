...
Cargo can build the CLI directly from a tag. The `--locked` flag is
non-negotiable — it pins to the exact `Cargo.lock` we shipped, which
matters because the workspace uses fat LTO (mismatched dep versions
silently change inlining behaviour).
...
freeze. Until then, depend on a tagged commit:
...
SemVer; breaking changes always advance the *major* version.
