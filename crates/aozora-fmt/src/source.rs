//! Guarded source-input reading, shared by both formatter frontends and the
//! `aozora` CLI.
//!
...
//!   the raw bytes fit, so [`encoding::decode`](crate::decode) re-checks the
//!   decoded length.
//!
...
#[must_use]
pub fn is_oversize_input(err: &anyhow::Error) -> bool {
    err.chain()
