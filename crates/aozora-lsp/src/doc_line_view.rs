...
//! line table ([`LineIndex::new`], an `O(doc)` `\n` scan) just to map a
...
//!
//! ## Byte-identity with [`LineIndex`]
//!
//! [`LineIndex`] counts **only** `\n` (`0x0A`) as a line break. ropey's
...
//! identical to [`LineIndex`]. The [`Rope`] arm here reproduces
//! [`LineIndex::position`] exactly, **including** the pre-existing skew
...
pub enum DocLineView<'a> {
    ...
    Rope(&'a Rope),
    /// Borrowed source plus its line index — the verbatim [`LineIndex`]
    /// path for callers that already hold a `&str`.
    Str {
...
impl<'a> DocLineView<'a> {
    /// Build a `&str`-backed view, computing the [`LineIndex`] once. Keeps
    /// today's behaviour for the one-shot and on-demand `&str` callers.
    #[must_use]
...
/// Reproduce [`LineIndex::position`] exactly over a LF-only [`Rope`].
///
/// `byte_to_line` assigns the `\n` to its own (left) line, matching
/// [`LineIndex`]'s `partition_point(start <= needle) - 1`; the column is
